...
void rt_handler(int signal, siginfo_t *info, void *arg __attribute__ ((__unused__)))
{
    ...
    //  unsafe to use fprintf() in a signal handler - but will suffice for demo
      fprintf(stderr, "Rx signal: Signnum %d  SIGRTMIN+%d, value: %d\n", signal,  signal - SIGRTMIN, val);
      return;
}
...
/*  The following 6 lines of code specify the set of signal which should be kept pending when rt_handler is executing */
/*  This avoid nested executions of the rt_handler code if multiple signals of the following 5 kinds are pending      */
    ...
    if (
        ...
        (sigaction(SIGRTMAX, &action, NULL) < 0) ) 
     {
    ...
    send_rt_signal(SIGRTMIN, 0);
    send_rt_signal(SIGRTMAX, 100);
